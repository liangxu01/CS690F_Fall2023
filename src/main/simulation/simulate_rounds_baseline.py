...
global_recommender = 0.02

def roundsSimulatorBaseline(nodeGraph, articleList, samplers):
    ...
    for index, article in enumerate(articleList):
        ...
        while True:
            ...
            for nodeIndex in range(consts.NODE_COUNT):
                ...
                print(nx.get_node_attributes(nodeGraphRound, "polInc"))

                node_party = nx.get_node_attributes(nodeGraphRound, "polInc")   
                article_party = article['political_inclination']

                if random.random() < global_recommender and node_party == article_party:
